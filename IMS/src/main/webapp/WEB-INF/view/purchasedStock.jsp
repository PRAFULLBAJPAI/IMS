...
<body style="font-size: 12px; background-color: #DCECFE;">
	...
	<div class="row mx-1">
		<div class="col-md-12">
			<div class="row">
				<div class="col-md-12">
					<h5 class="heading">Purchased Stock Report</h5>
				</div>
			</div>
		</div>
	</div>
					...
					<div class="m-1">
						<div class="table-container">
							...
							<table id="example" class="table table-striped table-bordered"
								...
								<tbody>
									<c:forEach items="${purchasedItemList.payload}"
										var="purchasedItem">
										<tr>
											<td class="left-align">${purchasedItem.itemCode}</td>
											<td class="left-align">${purchasedItem.itemName}</td>
											<td class="left-align">${purchasedItem.recievedDate}</td>
											<td class="left-align">${purchasedItem.invoiceDate}</td>
											<td class="right-align">${purchasedItem.invoiceNo}</td>
											<td class="left-align">${purchasedItem.vendorName}</td>
											<td class="right-align">${purchasedItem.itemQuantity}</td>
											<td class="right-align">${purchasedItem.lastPrice}</td>
											<td class="right-align">${purchasedItem.itemPrice}</td>
											<td class="left-align">${purchasedItem.entryDate}</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
		        ...
		        if (startDate !== '' && endDate !== '') {
		            $.fn.dataTable.ext.search.push(
		                function(settings, data, dataIndex) {
		                    var dateTime = data[9] || ""; // Use data for the date column
