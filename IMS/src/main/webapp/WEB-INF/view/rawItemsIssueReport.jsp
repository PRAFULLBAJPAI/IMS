...
<%@ page import="com.doritech.api.DTO.RawItemsIssueDTO"%>
...
<link rel="stylesheet" href="./css/rawItemsIssueReport.css" />
...
<body style="font-size: 12px; background-color: #DCECFE;">
	<div class="row mx-1">
		<div class="col-md-12">
			<div class="row">
				<div class="col-md-12">
					<h5 class="heading">Raw Items Issue Stock Report</h5>
				</div>
			</div>
		</div>
	</div>
							...
							<table id="example" class="table table-striped table-bordered"
								...
								<thead>
									<tr>
										...
										<th class="col-md-2">Consumption Type</th>
										<th class="col-md-2">Consumption For</th>
										<th class="col-md-1">Qty. in Stock</th>
										<th class="col-md-2">Min. Qty.</th>
										<th class="col-md-2">Item Qty.</th>
										<th class="col-md-2">Remarks</th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${rawItemsIssueList.payload}"
										var="rawItemsIssue">
										<tr>
											<td class="left-align">${rawItemsIssue.itemCode}</td>
											<td class="left-align">${rawItemsIssue.itemName}</td>
											<td class="left-align">${rawItemsIssue.issueDate}</td>
											<td class="left-align">${rawItemsIssue.consumptionType}</td>
											<td class="left-align">${rawItemsIssue.consumptionFor}</td>
											<td class="right-align">${rawItemsIssue.quantityInStock}</td>
											<td class="right-align">${rawItemsIssue.minimumQuantity}</td>
											<td class="right-align">${rawItemsIssue.itemQuantity}</td>
											<td class="left-align">${rawItemsIssue.remark}</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
	        ...
	        if (startDate !== '' && endDate !== '') {
	            $.fn.dataTable.ext.search.push(
	                function(settings, data, dataIndex) {
	                    var dateTime = data[2] || ""; // Use data for the date column
	...
	<script src="./js/rawItemsIssueReport.js"></script>
